...
import { Textarea } from "@/components/ui/textarea"; // Used by inline TargetEventRecallPhase
import { Input } from "@/components/ui/input";     // Used by inline TargetEventRecallPhase
import { toast } from "sonner";
import { useRecording } from "@/contexts/RecordingContext";

// Corrected import path for PredictionErrorSelector and its type
...
// TargetEventRecallPhase Component (Inline for TreatmentX.tsx pages)
...
const TargetEventRecallPhase: React.FC<TargetEventRecallPhaseProps> = ({ treatmentNumber, onComplete }) => {
  ...
  const [sessionSuds, setSessionSuds] = useState<string>(""); // SUDS input as string
  const handleSubmit = () => {
    if (!sessionTargetEventText.trim()) { toast.error("Please briefly describe or recall the target event for this session."); return; }
    const sudsNum = parseInt(sessionSuds, 10);
    if (isNaN(sudsNum) || sudsNum < 0 || sudsNum > 100) { toast.error("Please enter a valid SUDS score (e.g., 0-100)."); return; }
    toast.info(`Target Event & SUDS for Treatment ${treatmentNumber} recorded.`);
  ...
  return (
    <div className="space-y-6 p-6 border rounded-lg bg-card shadow-lg animate-fadeIn">
      <div className="space-y-2 text-center">
        <h2 className="text-2xl font-semibold text-primary">Step 1: Re-activate Target Memory</h2>
        <p className="text-sm text-muted-foreground">For Treatment {treatmentNumber}, briefly bring the original target event to mind. Describe key aspects or sensations below. Then rate your current Subjective Units of Distress (SUDS).</p>
      </div>
      <Textarea placeholder="Briefly describe the target event as you recall it now..." value={sessionTargetEventText} onChange={(e) => setSessionTargetEventText(e.target.value)} rows={5} className="bg-background text-foreground placeholder:text-muted-foreground/70 focus:ring-primary focus:border-primary"/>
      <div>
        <label htmlFor={`suds-input-t${treatmentNumber}`} className="block text-sm font-medium text-muted-foreground mb-1">Current SUDS Score (0-100) for this memory, right now:</label>
        <Input id={`suds-input-t${treatmentNumber}`} type="number" placeholder="e.g., 70" value={sessionSuds} onChange={(e) => setSessionSuds(e.target.value)} className="bg-background text-foreground placeholder:text-muted-foreground/70 focus:ring-primary focus:border-primary"/>
      </div>
      <Button onClick={handleSubmit} className="w-full" size="lg">Confirm for Treatment {treatmentNumber}</Button>
    </div>
  );
};
// End of TargetEventRecallPhase
...
The movie begins, showing a time when ${memory1}. Then the movie shows a time when ${sessionTargetEvent}. But then, ${errorObject.description}. You are noticing yourself being surprised and delighted with this change of scene. Then I see the movie ending on a moment when ${memory2}`;
    });
  ...
  if (isLoadingPage) { 
    return (<div className="flex justify-center items-center min-h-screen text-lg">Loading Treatment {THIS_TREATMENT_NUMBER} Data...</div>); 
  }
  if (sessionTargetEvent === null || sessionSuds === null) { 
    return (
      <div className="flex flex-col items-center justify-center min-h-screen text-lg p-4 text-center">
        <p className="mb-4">Error: Missing calibrated session data for Treatment {THIS_TREATMENT_NUMBER}.</p>
        <Button onClick={() => navigate(`/calibrate/${THIS_TREATMENT_NUMBER}`, { replace: true })}>
          Re-Calibrate Treatment {THIS_TREATMENT_NUMBER}
        </Button>
      </div>
    ); 
  }
      ...
      <div className="max-w-3xl mx-auto space-y-8">
        ...
        {showResultsView ? (
          ...
          <>
            ...
            {/* TargetEventRecallPhase is NOT rendered here anymore */}
