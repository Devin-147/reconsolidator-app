...
const FollowUp = () => {
  ...
  const handleSubmit = () => {
    ...
    if (completeTreatment && typeof initialOverallSuds === 'number') {
      completeTreatment("Follow-Up", currentSuds, initialOverallSuds); // <<< CORRECTED
  ...
  if (isSubmitted) {
    return (
      <div className="text-center p-8 space-y-6">
        ...
        <p className="text-lg text-muted-foreground">Your follow-up has been recorded.</p>
  ...
  return (
    <div className="space-y-8 p-4 md:p-6">
      <div className="text-center">
        ...
        <p className="text-muted-foreground mt-2">Final feedback and ratings.</p>
      </div>
      <div className="p-6 border rounded-lg bg-card space-y-4">
        <h2 className="text-xl font-semibold">Original Target Memory Review</h2>
        <p className="text-sm italic p-4 bg-muted/50 border rounded-md">{targetEventTranscript || "N/A"}</p>
        <p className="text-sm">Thinking about this original event now, rate your distress.</p>
      ...
      <div className="p-6 border rounded-lg bg-card space-y-4">
        ...
        <Textarea value={finalComments} onChange={(e) => setFinalComments(e.target.value)} placeholder="Your feedback is valuable..." rows={6} />
      </div>
      <div className="flex justify-end">
        <Button onClick={handleSubmit} size="lg">Submit Final Assessment</Button>
      </div>
    </div>
  );
};
