...
const Treatment1 = () => {
  ...
  console.log("Target Event Transcript:", targetEventTranscript);
      ...
      if (savedMemories) {
        try {
          ...
          console.log("Loaded memories from localStorage:", {
            ...
            targetEvent: state.targetEvent ? state.targetEvent.substring(0, 30) + "..." : "none",
          });
        } catch (error) {
  ...
  useEffect(() => {
    ...
    if (!effectiveMemory1 || !effectiveMemory2 || !effectiveTargetEvent || !memoriesSaved) {
      toast.error("Please record your memories and target event before starting Treatment 1");
      navigate("/");
    } else {
      console.log("Memories and target event are available, ready for Treatment 1");
      setCurrentPhase(0);
    }
...
The next scene in this movie cuts to me experiencing when ${targetEventDescription}. However, in this version, something different happens: ${error.description}
...
${memory2Text}`;
  ...
  const handleSave = () => {
    ...
    completeTreatment("Treatment 1", sudsLevel);

    toast.success("Treatment 1 responses saved successfully");
    navigate("/");
  };
