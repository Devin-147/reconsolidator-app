...
export const PhaseFive: React.FC<PhaseFiveProps> = ({ isCurrentPhase, selectedPredictionErrors, onComplete, treatmentNumber }) => {
  ...
  return (
    <div className="space-y-6 p-4 border rounded-lg">
      ...
      : (<div className="grid md:grid-cols-2 gap-4">{reversedScriptObjects.map((rs,rIdx)=>(<div key={rs.originalIndex} className="p-3 bg-black/10 rounded-lg space-y-2 border"><p className="text-xs"><span className="italic">{rs.title}</span> (Reversed)</p><div className="whitespace-pre-wrap text-sm p-2 bg-background rounded max-h-32 overflow-y-auto">{rs.reversedText}</div><h5>Your Recording:</h5><NarrationRecorder index={rIdx} onRecordingComplete={(url)=>handleUserReverseNarrationComplete(rIdx,url)} existingAudioUrl={userRecordedReverseAudios[rIdx]}/>{accessLevel==='premium_lifetime' && (<div><h5>AI Version:</h5>{rs.isLoadingAi && <div><Loader2 className="inline animate-spin"/>Loading...</div>}{rs.aiError && <Button onClick={()=>triggerSingleReverseAiLoad(rIdx)}>Retry</Button>}{rs.aiAudioUrl && !rs.isLoadingAi && !rs.aiError && <AnimatedLogoWithAudio audioUrl={rs.aiAudioUrl} width={100} height={100} playButtonText={`Play Rev ${rIdx+1}`} animationVariant={rIdx+1} forceIsPlaying={currentlyPlayingAiIndex===-(rIdx+1)} onTogglePlay={()=>handleToggleReverseAiPlayback(rIdx)}/>}</div>)}</div>))}</div>)}
