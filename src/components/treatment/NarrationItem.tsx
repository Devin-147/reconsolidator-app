...
interface NarrationItemProps { script: string; index: number; predictionErrorTitle: string; onRecordingComplete: (index: number, audioUrl: string | null) => void; existingAudioUrl: string | null; treatmentNumber: number; shouldAttemptAiLoad: boolean; onAiLoadAttemptFinished: (index: number) => void; }
export const NarrationItem = ({ script, index, predictionErrorTitle, onRecordingComplete, existingAudioUrl, treatmentNumber, shouldAttemptAiLoad, onAiLoadAttemptFinished }: NarrationItemProps) => {
  ...
  const generateOrSimulateAiNarration = useCallback(async () => {
    ...
    } else { onAiLoadAttemptFinished(index); }
  }, [userEmail, script, hasAttemptedLoad, accessLevel, index, onAiLoadAttemptFinished, treatmentNumber]);
  useEffect(() => { if (shouldAttemptAiLoad && !hasAttemptedLoad) { generateOrSimulateAiNarration(); } }, [shouldAttemptAiLoad, hasAttemptedLoad, generateOrSimulateAiNarration]);
  ...
  return (
    <div className="p-4 bg-card/80 border rounded-lg flex flex-col justify-between min-h-[420px]">
      ...
      <div className="mt-auto"><h4 className="text-sm">Record Your Narration:</h4><NarrationRecorder index={index} onRecordingComplete={handleUserRecordingCompletion} existingAudioUrl={existingAudioUrl} /></div>
      <div className="mt-4 pt-4 border-t min-h-[250px] flex flex-col items-center justify-center">
        ...
        {aiAudioError && !isLoadingAi && (<div className="text-red-500"><p>{aiAudioError}</p><Button onClick={generateOrSimulateAiNarration} variant="link" size="sm">Retry</Button></div>)}
        {!isLoadingAi && !aiAudioError && (<>
          ...
          ) : ( (accessLevel === 'trial' || accessLevel === 'standard_lifetime') && showTeaserReady && (<div className="text-center"><div style={{width:120,height:120}} className="opacity-40 mx-auto"><MyActualLogo/></div><Button onClick={handleUpgradeClick} className="mt-2"><Lock className="mr-2"/>Upgrade</Button></div>)
          )}
        </>)}
      </div>
    </div>
