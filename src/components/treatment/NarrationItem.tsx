...
export const NarrationItem = ({ script, index, predictionErrorTitle, onRecordingComplete, existingAudioUrl, treatmentNumber }: NarrationItemProps) => {
  ...
  return (
    <div className="p-4 bg-card/80 border rounded-lg flex flex-col min-h-[420px]">
      <div className="flex-grow"><h3 className="font-semibold text-primary">{predictionErrorTitle}</h3><div className="bg-muted/40 p-2 my-2 rounded max-h-32 overflow-y-auto"><pre className="text-sm whitespace-pre-wrap">{script}</pre></div></div>
      ...
      {(accessLevel === 'premium_lifetime' || accessLevel === 'trial' || accessLevel === 'standard_lifetime') && (
        <div className="mt-4 pt-4 border-t min-h-[250px] flex flex-col items-center justify-center">
          ...
          {aiAudioError && !isLoadingAi && (<div className="text-red-500"><p>{aiAudioError}</p><Button onClick={handleLoadAiNarration} variant="link">Retry</Button></div>)}
          {!isLoadingAi && !aiAudioError && (<>
            {aiAudioUrl && accessLevel === 'premium_lifetime' ? (
              <AnimatedLogoWithAudio audioUrl={aiAudioUrl} onPlaybackEnd={handleToggleAiPlayback} width={180} height={180} playButtonText={`Play: ${predictionErrorTitle}`} showLoadingText={true} animationVariant={index + 1} forceIsPlaying={isThisAiNarrationPlaying} onTogglePlay={handleToggleAiPlayback}/>
            ) : !aiAudioUrl && accessLevel === 'premium_lifetime' ? (
              <Button onClick={handleLoadAiNarration}>Load AI Narration & Animated Logo</Button>
            ) : (<div className="text-center"><div style={{width:120,height:120}} className="opacity-50 mx-auto"><MyActualLogo/></div><Button onClick={handleUpgradeClick} className="mt-2"><Lock className="mr-2"/>Upgrade to Use AI</Button></div>)}
          </>)}
        </div>
      )}
    </div>
