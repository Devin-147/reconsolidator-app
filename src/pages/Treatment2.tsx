...
const Treatment2 = () => {
  ...
  // All state variables are the same as Treatment1
  ...
  // All useEffects and handlers are identical to Treatment1
  useEffect(() => {
    ...
    if (locState?.sessionTargetEvent) {
      ...
      // NOTE: For T2+, we might not have neutralMemories/selectedErrors passed in state.
      // A more robust solution would fetch these from the DB if not in location.state.
      // For now, we assume they are passed for simplicity.
  ...
  const generateNarrativeScripts = useCallback(() => { /* Identical to T1 */ }, [memory1, memory2, sessionTargetEvent, selectedErrors]);
  ...
  const getPhaseTitle = () => { /* Identical to T1 */ };
