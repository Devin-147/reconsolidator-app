...
const Treatment3 = () => {
  ...
  console.log("Treatment3.tsx - Step:", currentProcessingStep, "Loading:", isLoadingPrereqs);

  // --- Corrected Prerequisite Check Effect ---
  useEffect(() => {
    console.log("T3 Prereq Check: Running. memoriesSaved:", memoriesSaved);
    const checkTimeout = setTimeout(() => {
      if (!memoriesSaved) {
        console.log("T3 Prereq Check: memoriesSaved is false. Redirecting to '/'.");
      ...
      if (!memory1 || !memory2 || !targetEventTranscript || typeof calibrationSuds !== 'number') {
        console.warn("T3 Prereq Check: memoriesSaved true, but required data missing. Redirecting to '/'.");
        toast.error("Memory data or initial SUDS error. Please save setup again.");
        navigate('/'); // Redirect back to setup page
        return;
      }
      console.log("T3 Prereq Check: Prerequisites met. Starting processing steps.");
      setIsLoadingPrereqs(false);
      setCurrentProcessingStep(0); // Start first processing step for T3
    }, 150); // Short delay
  ...
  const handlePredictionErrorsComplete = useCallback((errors: PredictionError[]) => { if (errors.length !== 11) { toast.error("Select 11 errors."); return; } console.log("T3: PEs selected:", errors.length); setSelectedErrors(errors); setCurrentProcessingStep(1); }, []);
  const generateNarrativeScripts = useCallback(() => { if (!memory1 || !memory2 || !targetEventTranscript || selectedErrors.length !== 11) { console.error("T3: Cannot generate scripts, data missing."); toast.error("Cannot generate: data missing."); return null; } console.log("T3: Generating scripts..."); const scripts = selectedErrors.map((error, index) => { /* ... template ... */ return `Movie ${index + 1}: ...`; }); console.log("T3: Scripts generated:", scripts.length); setNarrativeScripts(scripts); return scripts; }, [memory1, memory2, targetEventTranscript, selectedErrors]);
  ...
  const handleNarrationRecorded = useCallback((index: number, audioUrl: string | null) => { if (updateNarrationAudio) updateNarrationAudio(index, audioUrl); else console.error("T3: updateNarrationAudio missing."); }, [updateNarrationAudio]);
  ...
  const handleStep6Complete = useCallback((finalSuds: number) => { // Receives finalSuds
      if (completeTreatment && typeof calibrationSuds === 'number') {
          completeTreatment('Treatment 3', finalSuds); // <<< USE finalSuds ARGUMENT
          toast.success("Treatment 3 complete!");
          // Optional: Show results or navigate
          // setShowResultsView(true);
          navigate("/"); // Navigate home for now
      } else { console.error("T3: Cannot complete: ctx fn/calibrationSuds missing."); toast.error("Error saving results."); }
  ...
  return (
    <div className="min-h-screen bg-background p-6">
      ...
      <div className="max-w-3xl mx-auto space-y-8">
         ...
         {showResultsView ? (
             <div>Treatment 3 Results...</div>
         ) : (
            // --- Processing Steps View ---
            <>
              {currentProcessingStep !== null && (
                <div className="text-center space-y-2">
                  <h1 className="text-3xl font-semibold tracking-tight">Treatment 3</h1>
                  <p className="text-muted-foreground">Two nights after Treatment 2</p> {/* Update description */}
                </div>
              )}
              ...
              {currentProcessingStep === 4 && (<NarrationPhase isCurrentPhase={true} narrativeScripts={narrativeScripts} narrationAudios={narrationAudios?.filter(a=>a!==null) as string[]} onNarrationRecorded={handleNarrationRecorded} onComplete={handleStep4Complete} treatmentNumber={3}/>)}
              {currentProcessingStep === 5 && (<PhaseFive isCurrentPhase={true} narrativeScripts={narrativeScripts} memory1={memory1} memory2={memory2} targetEventTranscript={targetEventTranscript} predictionErrors={selectedErrors} onComplete={handleStep5Complete} treatmentNumber={3}/>)}
              ...
              {currentProcessingStep === 6 && (
                  <PhaseSix
                      ...
                      treatmentNumber={3} // Keep (Set correct number)
