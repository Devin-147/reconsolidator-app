...
const Treatment3 = () => {
  ...
  useEffect(() => {
    ...
    if ((!effectiveMemory1 || !effectiveMemory2 || !effectiveTargetEvent) && !isSaved) {
      toast.error("Please record your memories and target event before starting Treatment 3");
      navigate("/");
    } else {
      console.log("Memories and target event are available, ready for Treatment 3");
      setCurrentPhase(0);
    }
...
I see a version of me in the movie seat is surprised and delighted at this scene. 

And then it fades out and we see a new scene but this time:
...
${memory2Text}`;
  ...
  const handlePhase6Complete = (finalSuds: number) => {
    completeTreatment('Treatment 3', finalSuds);
    toast.success("Treatment 3 responses saved successfully");
    navigate("/");
  };
    ...
    <div className="min-h-screen bg-background p-6">
      ...
      <div className="max-w-3xl mx-auto space-y-8">
        <div className="text-center space-y-2">
          ...
          <p className="text-muted-foreground">Two nights after Treatment 2</p>
        </div>
